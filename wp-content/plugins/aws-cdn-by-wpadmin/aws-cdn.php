...
echo "<!--Amazon AWS CDN Plugin. Powered by WPAdmin.ca 1.5.8-->";
}
...
$wpa_cors_block = PHP_EOL . '##### Added By AWS CDN Plugin #####
<FilesMatch "\.(ttf|ttc|otf|eot|woff|woff2|font.css)$">
Header add Access-Control-Allow-Origin "*"
Header set Access-Control-Allow-Origin "*"
</FilesMatch>' . PHP_EOL;
$wpa_htfile = ABSPATH . ".htaccess";
...
if(file_exists($wpa_htfile))
{
$wpa_edata = file_get_contents($wpa_htfile);
if(!strpos($wpa_edata,'##### Added By AWS CDN Plugin #####') )
{
$wpa_edata = str_replace($wpa_cors_block,'',$wpa_edata);
file_put_contents($wpa_htfile, $wpa_edata );
}
}
...
if(!strpos($wpa_edata,'##### Added By AWS CDN Plugin #####') )
{
file_put_contents($wpa_htfile, $wpa_cors_block,FILE_APPEND );	
}
}
...
{
$wpa_domain = 	home_url();
$wpa_sitedomain = get_site_url();
if($wpa_domain <> $wpa_sitedomain)
{
	$wpa_domain = $wpa_sitedomain;
}
/* fix relative URLS*/
if(is_string($content)) 
{
$wpa_domain_name = str_replace("http://","",$wpa_domain);
$wpa_domain_name = str_replace("https://","",$wpa_domain);
...
if( strpos($content,'src="//' . $wpa_domain_name) )
{
$content = str_replace('src="//' . $wpa_domain_name ,'src="'.$wpa_domain, $content);
}
if( substr($content,0, strlen($wpa_domain_name) + 2) == '//' . $wpa_domain_name)
{
$content = str_replace('//' . $wpa_domain_name, $wpa_domain, $content);
}
}
/* fix relative URLS*/
...
add_filter('the_content',array($wpaawscdn,'wpaac_replace_content'),999);
add_filter( 'post_thumbnail_html', array($wpaawscdn,'wpaac_replace_content') ,999);
add_filter( 'widget_text', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'wp_get_attachment_link', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter('theme_root_uri',array($wpaawscdn,'wpaac_replace_content'),999);
add_filter('plugins_url',array($wpaawscdn,'wpaac_replace_content'),999);
add_filter( 'wp_get_attachment_thumb_file', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'wp_get_attachment_thumb_url', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'metaslider_resized_image_url', array($wpaawscdn,'wpaac_replace_content'),999);
add_filter( 'wp_get_attachment_url', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'wp_get_attachment_image_attributes', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'wp_get_attachment_image_srcset', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'woocommerce_product_get_image', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'post_gallery ', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'bloginfo', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'header_image', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'get_header_image_tag', array($wpaawscdn,'wpaac_replace_content'),999 );
add_filter( 'theme_mod_header_image', array($wpaawscdn,'wpaac_replace_content'),999 );
if(get_option('WPAdmin_CDN_SS') <> "checked")
{
add_filter( 'style_loader_src', array($wpaawscdn,'wpaac_replace_content') ,999);
}
if(get_option('WPAdmin_CDN_JS') <> "checked")
{
add_filter( 'script_loader_src', array($wpaawscdn,'wpaac_replace_content') ,999);
}
