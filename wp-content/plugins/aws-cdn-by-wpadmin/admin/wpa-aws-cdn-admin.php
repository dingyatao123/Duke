...
$nowwwdomain = str_replace('www.','',$wpa_domain);
...
if ( (is_multisite()  && is_super_admin()) || is_admin() ) 
{
echo '<div style="clear:both"></div><div class=col-sm-6>'. $wpa_notice .'</div><div class=col-sm-6>[ <a target=_BLANK href="http://' . $cdn_details . '">Test ' . $cdn_details . '</a> ] &nbsp;  [ <a target=_BLANK href="http://cdn.' . $nowwwdomain . '">Test cdn.' . $nowwwdomain . '</a> ]';
echo "<p>&nbsp;</p></div>";
}
?>
...
<div style='clear:both'></div>
<p>
<input type=checkbox id=wpaac_usecdn name=wpaac_usecdn> &nbsp; Instead of Amazon Cloudfront Domain name, I would like to use my custom domain: <b>cdn.<?php echo $nowwwdomain;?></b>
<div id=usecdn_notice style='display:none'>
<?php 
echo "<p><b>NOTE</b>: This feature needs an SSL certificate. The plugin will request a <b>Free</b> certificate from <a href='https://aws.amazon.com/certificate-manager/pricing/' target=_BLANK>Amazon Certificate Manager (ACM)</a>.</p><p>A verification email will be sent to these email addresses: <li>admin@$nowwwdomain</li><li>administrator@$nowwwdomain</li><li>hostmaster@$nowwwdomain</li><li>postmaster@$nowwwdomain</li><li>webmaster@$nowwwdomain</li></p><p>Please ensure you have any one of these emails address / alias configured.</p>";
?>
</p>
</div>
</div>
...
<li>Click the <b>Activate rAnd0mChA6s.cloudfront.net</b> OR <b>Activate cdn.<?php echo $nowwwdomain;?></b> button</li>
<li>Clear cache if you are using any caching plugin</li>
</ol>
...
var domainname = jQuery('#wpaac_cdnorigin').val();
domainname = domainname.replace('www.','');
jQuery('#wpaac_cdnorigin').val(domainname);
jQuery('#wpaac_cdnorigin').attr('value',domainname);
}
else{
jQuery("#usecdn_notice").hide();
jQuery('#wpaac_cdnorigin').val(<?php echo $wpa_domain; ?>);
jQuery('#wpaac_cdnorigin').attr('value','<?php echo $wpa_domain; ?>');
}
});

jQuery(".retval").bind('click',function(){
...
jQuery("#wpaac_validate").bind('click',function(){
var ak = jQuery("#wpaac_cdnak").val();
jQuery("#wpaac_cdnak").focus();
if(ak == "") return;
var sk = jQuery("#wpaac_cdnsk").val();
